(function () {
  ...
  function render() {
    ...
    users.forEach(u => {
      ...
      tr.innerHTML =
        ...
        '<td><div class="row-actions">' +
          (u.has_custom_theme ? '<button class="btn btn-ghost btn-sm" data-reset-theme="' + u.id + '" title="Reset this user\'s theme to default">Reset theme</button>' : '') +
    ...
    body.querySelectorAll('[data-reset-theme]').forEach(b => b.onclick = async () => {
      const uid = parseInt(b.dataset.resetTheme, 10);
      if (!confirm('Reset this user\'s theme to the default?')) return;
      try {
        await api('/api/users/' + uid + '/reset-theme', { method: 'POST' });
